...
/**
 * EnterprisePricingCard Component
 * 
 * A self-contained pricing card component for the Enterprise plan.
...
// Enterprise plan data defined directly in the component
const enterprisePlan: PricingPlan = {
  title: PRODUCT_IDS.enterprise.displayName,
  monthlyPrice: "$199",
  yearlyPrice: "$1,990",
  description: "For large organizations that need maximum scalability and support.",
  features: [
...
// Product IDs for the Enterprise plan
const productIds = {
  monthly: PRODUCT_IDS.enterprise.monthly,
  annual: PRODUCT_IDS.enterprise.annual
};
...
export default function EnterprisePricingCard({ 
  ...
  const annualCost = parseInt(plan.yearlyPrice.replace('$', '').replace(',', ''));
  ...
  return (
    <div className="bg-[#0d111b] rounded-2xl shadow-lg transition-all duration-300 p-8 border border-gray-800 hover:border-[#10b981]/30 hover:shadow-xl relative overflow-hidden hover:-translate-y-1">
      ...
      <div className="relative">
        ...
        {isCurrentPlan ? (
          ...
          <button
            ...
            disabled={isLoading}
            className="w-full py-3 rounded-lg bg-[#111827] hover:bg-[#1a2235] text-gray-300 py-3 px-6 rounded-xl font-medium transition-all duration-300 flex items-center justify-center border border-gray-700"
          ...
          <Link
            href={buttonHref}
            className="block w-full bg-[#111827] hover:bg-[#1a2235] text-gray-300 text-center py-3 px-6 rounded-xl font-medium transition-all duration-300 text-sm border border-gray-700"
            data-product-id={productId}
          >
            {plan.cta}
          </Link>
